...
void SuperLIO::init(){
  ...
  points_world_v3_.reserve(201000);
  abcd_vec_.resize(200000);
  effect_knn_idxs_.resize(200000);
...
bool SuperLIO::kf_init(){
  ...
  /// Accumulate enough IMU samples for a stable gravity estimate
  /// (frame count configurable via kf/imu_int_frame).
  if(imu_cout < g_imu_int_frame){
    return false;
  }
...
void SuperLIO::DynamicFilter(){
  ...
  const bool filtered = dyn_filter_->process(*ds_undistort_,
                                             ...
                                             *steady_cloud_world_);
  if(filtered){
    data_wrapper_->pub_mdet_result(dyn_cloud_world_, steady_cloud_world_,
                                   measures_.lidar.end_time);
  }
}
...
void SuperLIO::caceData(){
  ...
  if(g_if_filter || dyn_filter_->enabled()){
    // ds_undistort_ only holds static points once the moving-object filter ran.
