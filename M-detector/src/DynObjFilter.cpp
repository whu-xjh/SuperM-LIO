...
void  DynObjFilter::init(ros::NodeHandle& nh)
{
    ...
    // NOTE: these two keys are crossed on purpose (upstream behavior): the value
    // under ver_resolution_max becomes the AZIMUTH cell size (loaded into the
    // hor_resolution_max variable), and hor_resolution_max the ELEVATION cell
    // size. Do not "fix" this without moving the yaml values back in step.
